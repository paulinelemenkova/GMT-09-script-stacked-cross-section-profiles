...
gmt psbasemap -R -J \
	...
	-Bxg4f2a4 -Byg4f2a4 \
	-B+t"Cross-sectional profiles of the Kuril-Kamchatka Trench: northern part" -O -K >> $ps
...
gmt psxy -R-200/200/-10000/0 -JX6i/2i \
	...
	-Glightgray env2.txt -Y7.5i -O -K >> $ps
gmt psxy -R -J -W1p -Ey stack2.txt -O -K >> $ps
gmt psxy -R -J -W1p,red stack2.txt -O -K >> $ps
# Step-14. Add test annotations
echo "100 -7000 Pacific Plate" | gmt pstext -R -J -Gwhite -F+jTC+f8p -O -K >> $ps
echo "50 -300 Median stacked profile with error bars" | gmt pstext -R -J -Gwhite -F+jTC+f10p,red -O -K >> $ps
echo "0 -8000 Kuril-Kamchatka Trench" | gmt pstext -R -J -Gwhite -F+jTC+f8p -O -K >> $ps
echo "-150 -5000 Greater Kuril Chain" | gmt pstext -R -J -Gwhite -F+jTC+f8p -O -K >> $ps
# Step-15. Add GMT logo
gmt logo -Dx6.2/-21.0+o0.1i/0.1i+w2c -O >> $ps
#gmt psxy -R -J -O -T >> $ps
# Step-16. Clean up (not necessary in this case)
